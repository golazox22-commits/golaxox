...
# Color filter palette: (key, label, hex).
# First 6 are visible; the rest appear behind "المزيد +".
...
def prod_json(p):
    ...
    return {
        ...
        "club_ar": cfg.club_name(p, False),
        "club_en": cfg.club_name(p, True),
        "name_ar": p["name_ar"],
        "name_en": p["name_en"],
...
def gx_data():
    ...
    return {
        ...
        "sizes": cfg.SIZE_ORDER,
        "chart": cfg.SIZE_CHART,
        "rewards": cfg.REWARDS,
        "products": products,
        "clubs": clubs,
        "points_per": cfg.POINTS_PER_BHD,
        "match": m,
        "drop": d,
        "poll": poll,
...
def match_info():
    ...
    if now < ko - datetime.timedelta(
        hours=cfg.MATCHDAY_START_HOURS
    ):
        return None

    if now > ko + datetime.timedelta(
        hours=cfg.MATCHDAY_END_HOURS
    ):
        return None
...
def drop_info():
    ...
    return {
        ...
        "en": d.get("en", ""),
        "img": d.get("img", ""),
        "ids": d.get("product_ids", []),
        "target_iso": tg.isoformat(),
...
def poll_active():
    ...
    if p["status"] == "open":
        ...
        return {
            ...
            "start": p["start"],
            "end": p["end"],
            "votes": db.votes_count(p["id"]),
            "total": db.votes_total(p["id"]),
            "ended": bool(en and now > en),
        }
...
def seed_super_admin():
    phone = fix_phone(
        os.environ.get("SUPER_ADMIN_PHONE")
...
def order_stage(o):
    ...
    return (
        ORDER_FLOW.index(status) if status in ORDER_FLOW else -1,
        status == "delivered"
    )
...
def img(name):
    ...
    p = next(
        (x for x in cfg.PRODUCTS if x["id"] == base),
        None
    )
...
:root {
    ...
    --sh: 0 8px 28px rgba(15,23,42,.08);
    --sh2: 0 18px 44px rgba(15,23,42,.14);
...
html[data-theme="dark"][data-club] .hero {
    background:
        linear-gradient(
            120deg,
            var(--tint, rgba(225,29,72,.06)) 0%,
...
.tryme {
    ...
    margin-top: 10px;
    padding: 8px 10px;
    border: 1.5px dashed var(--ac, #E11D48);
...
html[data-theme="dark"] .tbadge.warn {
    background: #3B1D0B;
    border-color: #7C2D12;
    color: #FDBA74;
}
...
(function() {
    ...
    document
        .querySelectorAll("[data-font-btn]")
        .forEach(function(b) {

            b.addEventListener("click", function() {
                setFont(
                    b.getAttribute("data-font-btn")
                );
            });
    ...
    var qty =
        document.getElementById("qtyBox");

    if (qty) {

        var qn =
            qty.querySelector(".qn");

        var minus =
            qty.querySelector(".qminus");

        var plus =
            qty.querySelector(".qplus");
    ...
    document
        .querySelectorAll(".size-chip[data-size]")
        .forEach(function(chip) {

            chip.addEventListener("click", function() {

                if (chip.classList.contains("oos")) {
                    return;
                }
 ...
 def page(title, body, extra_head=""):
    ...
    nav_html = "".join(
        '<a class="nv" href="%s">%s</a>' % (h, n)
        for h, n in nav_items
    )

    admin_link = (
        '<a class="hbtn" href="/admin">🛠️ %s</a>'
        % ("لوحة التحكم" if lg == "ar" else "Admin")
        if admin else ""
    )
...
<header class="hd">
  <div class="hd-in">
    <a class="logo" href="/home">
      <span class="ball">⚽</span> golazox
    </a>

    <nav class="nav" id="mainNav">%s</nav>

    <a class="hbtn hicon" href="/cart">
      🛒 %s
      <span class="hcount" style="%s">%s</span>
    </a>
...
%s
        ...
        extra_head,
        nav_html,
        "السلة" if lg == "ar" else "Cart",
        "display:flex" if cart_count else "display:none",
        cart_count,
        admin_link,
        body,
        t("badge"),
