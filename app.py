...
def gx_data():
    products = [prod_json(p) for p in cfg.PRODUCTS]
...
def seed_super_admin():
    ...
    if not u:
        return db.user_create(phone, name, "super_admin")
...
/* ---------- search & filters ---------- */
var filters={club:'all',type:'all',size:'all',fav:false};
...
function clearFilters(){
  filters={club:'all',type:'all',size:'all',fav:false};
...
function selectSize(el){
  if(el.classList.contains('oos')){ toast(gxT('size_oos')); return; }
...
function authVerify(){
  ...
  .then(function(r){return r.json();}).then(function(d){
    if(d.ok){ closeModal('m-login'); location.href='/account'; } else { toast(gxT('auth_wrong')); }
  });
}
...
def footer_html():
    d = cfg.L[lang()]
    links = ("<a onclick='openModal(\"m-sizes\")'>{0}</a><a onclick='openModal(\"m-wash\")'>{1}</a>"
             "<a onclick='openModal(\"m-ret\")'>{2}</a><a onclick='openModal(\"m-how\")'>{3}</a>"
             "<a onclick='openModal(\"m-contact\")'>{4}</a>").format(
...
def modals_html():
    ...
    login_body = ('<div class="auth-box">'
                  ...
                  '<button class="btn pri big" onclick="authSendCode()">{s}</button></div>'
                  '</div>'
                  ).format(sub=d["auth_sub"], ph=d["auth_phone"], otp=d["auth_otp_ph"],
                           nm=d["auth_name_ph"], new=d["auth_new"], demo=d["auth_demo_note"],
                           v=d["auth_verify"], s=d["auth_otp_btn"])
...
def home_body():
    ...
    searchbar = ('<div class="sb">'
                 ...
                 '<button class="chip" onclick="clearFilters()">{clear}</button></div>'
                          ...
                          clubs=club_opts, types=type_opts, sizes=size_opts, fav=d["fav_filter"], clear=d["clear_filters"])

    jgrid = "".join(product_card(p) for p in cfg.PRODUCTS if p["kind"] == "jersey")
    mgrid = "".join(product_card(p) for p in cfg.PRODUCTS if p["kind"] == "mug")
    ...
    if pol:
        ...
        hide = bool(data.get("hide") and not pol["ended"] and pol["status"] == "open")
        dev = None
        ...
        if pol["status"] == "closed" or pol["ended"]:
            ...
            for o in options:
                votes = pol["votes"].get(str(o.get("id")) if o.get("id") is not None else o.get("key"), 0) if isinstance(pol["votes"], dict) else 0
    ...
    quick = ('<div class="qcard" onclick="openModal(\'m-sizes\')"><div class="qic">📏</div>'
    ...
    return ('<div class="wrap">'
            '<div class="hero"><h1>{t1}<br><span class="g">{t2}</span></h1><p>{sub}</p>'
            '<div class="hero-btns"><a class="btn pri" href="javascript:void(0)" onclick="goSec(\'jerseys\')">{cj}</a>'
            '<a class="btn ghost" href="javascript:void(0)" onclick="goSec(\'mugs\')">{cm}</a></div></div>'
            + searchbar
            ...
            + '</div>'
                     ...
                     sn=d["search_none"], sj=d["sec_jerseys"], sj_sub=d["sec_jerseys_sub"],
...
def product_card(p):
    ...
    first = p["imgs"][0]
    return (
        '<div class="pcard" data-id="{id}" data-kind="{kind}" data-club="{cid}" data-clubn="{cn}" data-stock="{csv}" data-name="{name}">'
        ...
        '<div class="pfoot"><b>{pr}</b><a class="pview" href="/product/{id}">{view} ←</a></div></div></div>'
             ...
             name=name.replace('"', "&quot;"), badges=badges_html, on="on" if fav else "", h="❤" if fav else "🤍",
